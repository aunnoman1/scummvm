...
static const EfhSubMenuOption EfhSubMenuOptions[] = {
...
Common::KeymapArray EfhMetaEngine::initKeymaps(const char *target) const {
	...
	for (const EfhSubMenuOption *i = EfhSubMenuOptions; i->id; ++i) {
		act = new Action(i->id, _(i->desc));
		act->setCustomEngineActionEvent(i->action);
		act->addDefaultInputMapping(i->inputKey);
	...
	// I18N: This action is used to view the terrain during a fight so that the player can see where they can move if they decide to run.
	...
	keymap[5] = statusMenuKeymap;
	keymap[6] = statusMenuSubMenuKeymap;
	keymap[7] = interactionKeymap;
	keymap[8] = fightKeymap;
	keymap[9] = characterSelectionKeymap;
	keymap[10] = enemySelectionKeymap;
	keymap[11] = deathMenuKeymap;
